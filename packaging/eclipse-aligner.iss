; Windows installer for eclipse-aligner (Inno Setup).
;
...
; dist\eclipse-aligner\, and writes dist\version.txt. This wraps that folder
...
#define AppName "eclipse-aligner"
#define SrcDir  "dist\eclipse-aligner"
...
AppId={{6F5E4C1A-2B7D-4E9F-A3C8-1D2E3F4A5B6C}
AppName={#AppName}
AppVersion={#AppVer}
...
; but it is what this is under, so it is put in front of the person rather than
; buried in a subfolder they will never open.
LicenseFile=..\LICENSE
...
UninstallDisplayIcon={app}\eclipse-aligner.ico
...
OutputDir=dist
OutputBaseFilename=eclipse-aligner-{#AppVer}-x64-setup
Compression=lzma2
SolidCompression=no
WizardStyle=modern
...
Name: "{group}\{#AppName}"; Filename: "{app}\python\pythonw.exe"; Parameters: "-m eclipse_aligner"; WorkingDir: "{app}"; IconFilename: "{app}\eclipse-aligner.ico"
Name: "{group}\Uninstall {#AppName}"; Filename: "{uninstallexe}"
Name: "{autodesktop}\{#AppName}"; Filename: "{app}\python\pythonw.exe"; Parameters: "-m eclipse_aligner"; WorkingDir: "{app}"; IconFilename: "{app}\eclipse-aligner.ico"; Tasks: desktopicon

[Run]
; Offer to launch on finish.
Filename: "{app}\python\pythonw.exe"; Parameters: "-m eclipse_aligner"; WorkingDir: "{app}"; Description: "Launch {#AppName}"; Flags: nowait postinstall skipifsilent
